...
public class ApiController implements CommandLineRunner {
    ...
    private void mostrarTop10Libros() {
        List<Libro> topLibros = databaseService.obtenerTop10LibrosMasDescargados();
        ...
        if (topLibros.isEmpty()) {
            ...
            for (int i = 0; i < topLibros.size(); i++) {
                System.out.println((i + 1) + ". " + topLibros.get(i).getTitulo() +
                        " - Descargas: " + topLibros.get(i).getDescargas());
            }
        }
    }

    private void buscarAutorPorNombre() {
        System.out.print("\nIngrese el nombre o parte del nombre del autor: ");
        String nombre = scanner.nextLine();

        List<Autor> autores = databaseService.buscarAutoresPorNombre(nombre);
